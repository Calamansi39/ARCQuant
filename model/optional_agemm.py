...
_FORCE_DISABLE = os.getenv("ARC_LBC_DISABLE_AGEMM", "").strip().lower() in {"1", "true", "yes"}

agemm, AGEMM_IMPORT_ERROR = _try_import_agemm()
DEVICE_SM = _device_sm()

# kernels/CMakeLists.txt builds for SM120a only.
...
def warn_agemm_fallback_once(reason: str) -> None:
    ...
    _warned = True
    warnings.warn(
        f"agemm is unavailable/disabled ({reason}); falling back to fake NVFP4 quantization + PyTorch ops. "
        "Set ARC_LBC_DISABLE_AGEMM=0 and run on SM120+ to enable kernels.",
        RuntimeWarning,
        stacklevel=2,
    )
...
def require_agemm() -> ModuleType:
    if HAS_AGEMM and agemm is not None:
        return agemm
    if _FORCE_DISABLE:
        raise RuntimeError("agemm is disabled by ARC_LBC_DISABLE_AGEMM=1")
